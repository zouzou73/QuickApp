...
export class ProvidersComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.columns = [
            ...
            { prop: 'name', name: 'PatientName', cellTemplate: this.nameTemplate, width: 200 },
            { prop: 'description', name: 'Observation', cellTemplate: this.descriptionTemplate, width: 500 },
    ...
    fetch(cb) {
        ...
        if (data == null) {
            setTimeout(() => {
                ...
                if (data == null) {
                    data = [
                        { "completed": true, "important": true, "name": "Subscribe medicine", "description": "Create a visual studio VSIX extension package that will add this project as an aspnet-core project template" },
    ...
    delete(row) {
        this.alertService.showDialog('Are you sure you want to delete the patient?', DialogType.confirm, () => this.deleteHelper(row));
    }
    ...
    getFromDisk() {
        return this.localStorage.getDataObject(`${ProvidersComponent.DBKeyTodoDemo}:${this.currentUserId}`);
    }

    saveToDisk() {
        if (this.isDataLoaded)
            this.localStorage.saveSyncedSessionData(this.rowsCache, `${ProvidersComponent.DBKeyTodoDemo}:${this.currentUserId}`);
    }
}
